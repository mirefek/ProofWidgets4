...
export default function (props: any) {
  ...
  const last = React.useRef<rubikscolor[][]|null>(null)
  ...
  async function find_move(cube1 : rubikscolor[][], cube2 : rubikscolor[][]) {
    ...
    var startT = -1.0
    if (""+last.current == ""+cube2)
      startT -= t;
    // console.log(`startT = ${startT}`)
    cancel_animation()
    last.current = cube1
    if (m == "")
      setPropsD("cancel", {cube:cube2, time:0.0, move:""})
    else
      setPropsD("activate", {cube:cube2, time:startT, move:m})
  }

  if (""+props.cube != ""+cube.current) {
    // console.log("Input")
    // console.log(props.cube)
    const cube1 = cube.current
    cube.current = props.cube
    find_move(cube1, props.cube)
  }
